...
Cilindro::Cilindro(const Vector3& _posicion, const float _radio) :
	 posicion(_posicion), radio(_radio)
{
	setBoundingBox();
}
Cilindro::Cilindro(const Vector3& _posicion, const float _radio,std::shared_ptr<Textura> _tex) :
...
//
// Cilindro::Cilindro(const Vector3& _posicion, const float _radio, const std::string fichero_textura) :
...
std::string Cilindro::to_string() const {
	return "--- Cilindro:\nposicion: " + posicion.to_string() + "\nradio: " + std::to_string(radio);
}

Vector3 Cilindro::getPos() const {
	return posicion;
}

float Cilindro::getRadio() const {
	return radio;
}


// True sii el rayo desde <origen>, hacia <dir> intersecta con la Cilindro
...
std::optional<Figura::InterseccionData> Cilindro::interseccion(const Vector3& origen, const Vector3& dir) const
{
...
Vector3 Cilindro::getNormal(const Vector3& pto) const {
...
/*void Cilindro::addTextura(Textura _tex){ //NO FUNCIONA
	tex=_tex;
	std::cout<<"que pasa"<<std::endl;
	textura=true;
}*/

void Cilindro::setBoundingBox() {
...
std::shared_ptr<Prisma> Cilindro::getBoundingBox() const {
	return bbox;
}
