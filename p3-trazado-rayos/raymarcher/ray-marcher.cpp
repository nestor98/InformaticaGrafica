...
Color RayMarcher::shadowRay(const Vector3& pto, const Vector3& normal, const int indiceluz) const {
	Color c;
...
Color RayMarcher::luzDirecta(const Vector3& pto, const Vector3& normal) const {
	Color L;
...
Color RayMarcher::muestraLuzDirecta(const Vector3& pto, const Vector3& normal,
...
//////////////////////////////////////////////////////////////
*/
Color RayMarcher::ruletaRusa(const std::shared_ptr<Figura> fig, const Vector3& dir,
...
Vector3 vectorTipoRender(const RayMarcher::TipoRender tipoRender, const std::shared_ptr<Figura> fig, const Vector3& dir, const Vector3& ptoInterseccion, const GeneradorAleatorio& gen) {
	Material mat = fig->getMaterial();
	if (tipoRender == RayMarcher::VectoresWiRefraccion){
	...
	else if (tipoRender == RayMarcher::VectoresWiReflexion) {
		Matriz4 base = fig->getBase(ptoInterseccion);
		return mat.getVectorSalida(base, gen, 1, dir);
	}
	else if (tipoRender == RayMarcher::VectoresWiDifusos) {
		Matriz4 base = fig->getBase(ptoInterseccion);
		return mat.getVectorSalida(base, gen, 0, dir);
	}
	else if (tipoRender == RayMarcher::Normales) {
